    ...
    <div class="w-[95%] mx-auto ">
        <div class="container mx-auto mt-3">
            <div class="flex items-center justify-between">
                <h2 class="text-2xl font-bold mb-4">Hot Package</h2>
                ...
                <table id="example" class="min-w-full bg-white border border-gray-200">
                    <thead>
                        <tr class="bg-gray-100">
                            ...
                            <th class="px-4 py-2 text-left">discounts%</th>
                            <th class="px-4 py-2 text-left">btn_link</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <form action="{{ route('hot-package.update', $slider->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <td class="px-4 py-2">{{ $slider->id }}</td>
                                <td class="px-4 py-2">
                                    @if($slider->image)
                                        <img src="{{ asset($slider->image) }}" class="w-20 h-20 object-cover" alt="Slider Image">
                                    @endif
                                ...
                                <td class="px-4 py-2">
                                    <textarea name="discounts" rows="2"
                                        class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $slider->discounts }}</textarea>
                                </td>
                                <td class="px-4 py-2">
                                    <textarea name="btn_link" rows="2"
                                        class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $slider->btn_link }}</textarea>
                                </td>
                            ...
                            <form action="{{ route('hot-package.destroy', $slider->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
            ...
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-6 space-y-6">
                        <form action="{{ route('hot-package.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="grid gap-4 grid-cols-1 sm:grid-cols-2">
                                ...
                                <div class="sm:col-span-2">
                                    <label for="discounts" class="block text-sm font-medium text-gray-700">discounts% (ONLY PROVIDE % AMOUNT)</label>
                                    <textarea id="discounts" name="discounts" rows="2"
                                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm resize-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"></textarea>
                                </div>
                                <div class="sm:col-span-2">
                                    <label for="btn_link" class="block text-sm font-medium text-gray-700">btn_link</label>
                                    <textarea id="btn_link" name="btn_link" rows="2"
                                        class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm resize-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"></textarea>
                                </div>
