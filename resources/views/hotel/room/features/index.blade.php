<x-app-layout>
    <div class="container w-[95%] mx-auto">
        <div class="flex item-center justify-between px-10 pt-10">
            <h1 class="text-2xl font-bold ">Room Facilities: </h1>
                            ...
                            <div id="featureFields">
                                <div class="featureRow grid grid-cols-1 gap-4 mb-4">
                                    <div class="form-group">
                                        <label for="name" class="block text-sm font-medium text-gray-700">Feature Name 
                                    ...
                                    <div class="form-group">
                                        <label for="category_id" class="block text-sm font-medium text-gray-700">Category 
                                            <span class="text-red-500 font-bold text-2xl">*</span></label>
                                        <select name="category_id" class="form-control mt-1 p-2 border border-gray-300 rounded-md w-full" required>
            ...
            <table id="features-table" class="table-auto w-full border-collapse">
                <thead>
                    <tr class="bg-gray-100">
                        ...
                        <th class="px-4 py-2 border">Facilities Name</th>
                ...
                <tbody>
                    ...
                    <tr class="bg-white hover:bg-gray-50">
                        ...
                        <form action="{{ route('features.update', $feature->id) }}" method="POST" class="update-form">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="type" value="hotel">
                            ...
                            <td class="px-4 py-2 border">
                                <textarea name="name"
                                    ...
                                    disabled>{{ $feature->name }}</textarea>
                            </td>
                            <td class="px-4 py-2 border">
                                <select name="category_id" class="w-full border border-gray-300 rounded px-2 py-1" disabled>
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ $feature->category_id == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <td class="px-4 py-2 border">
                                <select name="isactive" class="w-full border border-gray-300 rounded px-2 py-1" disabled>
                                    <option value="1" {{ $feature->isactive ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ !$feature->isactive ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </td>
                            ...
                            <!-- Delete Button -->
                            <form action="{{ route('features.destroy', $feature->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
