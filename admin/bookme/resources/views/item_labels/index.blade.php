<x-app-layout>
    <div class="container w-[95%] mx-auto">
        <div class="flex items-center justify-between px-10 pt-10">
            <h1 class="text-2xl font-bold">Item Labels</h1>

            <!-- Add Label Button -->
            <div class="float-right">
                <button data-modal-target="label-modal" data-modal-toggle="label-modal"
        ...
        <!-- Modal for Adding Item Labels -->
        <div id="label-modal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                <div class="relative bg-white rounded-lg shadow">
                    <!-- Modal header -->
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                        <h3 class="text-xl font-semibold text-gray-900">Add Item Label</h3>
                        <button type="button"
                            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
                            data-modal-hide="label-modal">
                    ...
                    <!-- Modal body -->
                    <form action="{{ route('item-labels.store') }}" method="POST">
                        @csrf
                        <div class="p-4 md:p-5 space-y-4">
                            <div class="grid grid-cols-1 gap-4 mb-4">
                                <div class="form-group">
                                    <label for="name" class="block text-sm font-medium text-gray-700">
                                        Label Name <span class="text-red-500 font-bold">*</span>
                                    </label>
                        ...
                        <div class="flex items-center px-4 md:p-5 border-t border-gray-200 rounded-b">
                            ...
                            <button type="button" data-modal-hide="label-modal"
            ...
            <table class="table-auto w-full border-collapse">
                <thead>
                    <tr class="bg-gray-100">
                        ...
                        <th class="px-4 py-2 border">Label Name</th>
                ...
                <tbody>
                    @foreach($itemLabels as $label)
                        <tr class="bg-white hover:bg-gray-50">
                            <form action="{{ route('item-labels.update', $label->id) }}" method="POST" class="update-form">
                                @csrf
                                @method('PUT')
                                <td class="px-4 py-2 border">{{ $label->id }}</td>
                                <td class="px-4 py-2 border">
                                    <textarea name="name"
                                        class="w-full border border-gray-300 rounded px-2 py-1 resize-none"
                                        disabled>{{ $label->name }}</textarea>
                                </td>
                            ...
                            <!-- Delete Button -->
                            <form action="{{ route('item-labels.destroy', $label->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
