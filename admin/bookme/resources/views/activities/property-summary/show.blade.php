<x-app-layout>
  ...
  >
            ...
            <div class="relative p-4 w-full max-w-5xl max-h-full">
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                        <h3 class="text-xl font-semibold text-gray-900">Add Property Summary</h3>
                    ...
                    <form action="{{ route('property-summary.store') }}" method="POST" id="summaryForm">
                        ...
                        <div class="p-4 md:p-5 space-y-4">
                            <div id="summaryFields">
                                @php
                                    $defaultNames = ['Duration', 'person allow', 'Policy'];
                                    $defaultIcons = ['FaRegClock', 'PiUsersThree', 'MdOutlineFreeCancellation'];
                                    $defaultLabels = ['duration', 'person allow', 'policy'];
                                @endphp
                                ...
                                @foreach ($defaultNames as $index => $name)
                                    <div class="summaryRow grid grid-cols-4 gap-4 mb-4">
                                        ...
                                        <div class="form-group">
                                            ...
                                            @if($defaultNames[$index] === 'Duration')
                                                <div class="duration-fields flex space-x-2">
                                                    <div class="w-1/2">
                                                        <label class="block text-xs text-gray-500 mb-1">From</label>
                                                        <select name="duration_from_hours[]" class="w-full p-2 border border-gray-300 rounded-md duration-select" required>
                                                            <option value="">Hours</option>
                                                            @for($i = 0; $i <= 24; $i++)
                                                                <option value="{{ $i }}h">{{ $i }}h</option>
                                                            @endfor
                                                        </select>
                                                        <select name="duration_from_minutes[]" class="w-full p-2 border border-gray-300 rounded-md mt-1 duration-select" required>
                                                    ...
                                                    <div class="w-1/2">
                                                        ...
                                                        <select name="duration_to_hours[]" class="w-full p-2 border border-gray-300 rounded-md duration-select" required>
                                                            <option value="">Hours</option>
                                                            @for($i = 0; $i <= 24; $i++)
                                                                <option value="{{ $i }}h">{{ $i }}h</option>
                                                            @endfor
                                                        </select>
                                                        <select name="duration_to_minutes[]" class="w-full p-2 border border-gray-300 rounded-md mt-1 duration-select" required>
                                                            <option value="">Minutes</option>
                                                            @for($i = 0; $i < 60; $i += 15)
                                                                <option value="{{ $i }}m">{{ $i }}m</option>
                                                            @endfor
                                                ...
                                                <input type="hidden" name="value[]" class="duration-value" value="">
                                            @else
                                                <input type="text" name="value[]" class="mt-1 p-2 border border-gray-300 rounded-md w-full" placeholder="Enter {{ $defaultLabels[$index] }}" required>
                                            @endif
                                        </div>
                        ...
                        <form action="{{ route('property-summary.update', $summary->id) }}" method="POST" class="update-form">
                            ...
                            <td class="px-4 py-2 border">
                                ...
                                @if($currentName === 'Duration')
                                    @php
                                        // Parse duration value (assuming format like "1h 30m to 4h")
                                        $durationParts = explode(' to ', $summary->value);
                                        $durationFrom = $durationParts[0] ?? '';
                                        $durationTo = $durationParts[1] ?? '';
                                        
                                        // Extract hours and minutes from from value
                                        $fromParts = explode(' ', $durationFrom);
                                        $fromHours = 0;
                                        $fromMinutes = 0;
                                        foreach ($fromParts as $part) {
                                            if (strpos($part, 'h') !== false) {
                                                $fromHours = intval($part);
                                            } elseif (strpos($part, 'm') !== false) {
                                                $fromMinutes = intval($part);
                                            }
                                        }
                                        
                                        // Extract hours and minutes from to value
                                        $toParts = explode(' ', $durationTo);
                                        $toHours = 0;
                                        $toMinutes = 0;
                                    ...
                                    <div class="duration-fields flex space-x-2">
                                        <div class="w-1/2">
                                            <label class="block text-xs text-gray-500 mb-1">From</label>
                                            <select name="duration_from_hours" class="w-full p-1 border border-gray-300 rounded duration-select" disabled>
                                                <option value="">Hours</option>
                                                @for($i = 0; $i <= 24; $i++)
                                                    <option value="{{ $i }}h" {{ $fromHours == $i ? 'selected' : '' }}>{{ $i }}h</option>
                                                @endfor
                                            </select>
                                            <select name="duration_from_minutes" class="w-full p-1 border border-gray-300 rounded mt-1 duration-select" disabled>
                                                <option value="">Minutes</option>
                                                @for($i = 0; $i < 60; $i += 5)
                                                    <option value="{{ $i }}m" {{ $fromMinutes == $i ? 'selected' : '' }}>{{ $i }}m</option>
                                                @endfor
                                            </select>
                                        </div>
                                        <div class="w-1/2">
                                            <label class="block text-xs text-gray-500 mb-1">To</label>
                                            <select name="duration_to_hours" class="w-full p-1 border border-gray-300 rounded duration-select" disabled>
                                                <option value="">Hours</option>
                                                @for($i = 0; $i <= 24; $i++)
                                                    <option value="{{ $i }}h" {{ $toHours == $i ? 'selected' : '' }}>{{ $i }}h</option>
                                                @endfor
                                            </select>
                                            <select name="duration_to_minutes" class="w-full p-1 border border-gray-300 rounded mt-1 duration-select" disabled>
                                                <option value="">Minutes</option>
                                                @for($i = 0; $i < 60; $i += 5)
                                                    <option value="{{ $i }}m" {{ $toMinutes == $i ? 'selected' : '' }}>{{ $i }}m</option>
                                                @endfor
                                            </select>
                                        </div>
                                    </div>
                                    <input type="hidden" name="value" class="duration-value" value="{{ $summary->value }}">
                                @else
    ...
    <script>
        ...
        // Function to toggle duration fields based on facility name selection
        function toggleDurationFields(selectElement) {
            ...
            if (facilityName === 'Duration') {
                // Remove existing value field if it exists
                const existingValueField = valueContainer.querySelector('input[name="value[]"]');
                ...
                // Add event listeners to update the hidden value field
                const durationSelects = valueContainer.querySelectorAll('.duration-select');
                durationSelects.forEach(select => {
                    select.addEventListener('change', updateDurationValue);
                });
            } else {
                // Remove duration fields if they exist
                const durationFields = valueContainer.querySelector('.duration-fields');
                if (durationFields) {
                    durationFields.remove();
                }
                
                // Remove the hidden duration value field if it exists
                const durationValueField = valueContainer.querySelector('.duration-value');
                if (durationValueField) {
                    durationValueField.remove();
                }
                
                // Create regular value field
                valueContainer.innerHTML = `
                    <label class="block text-sm font-medium text-gray-700">Value<span class="text-red-500 font-bold">*</span></label>
                    <input type="text" name="value[]" class="mt-1 p-2 border border-gray-300 rounded-md w-full" placeholder="Enter value" required>
                `;
            }
        }

        // Function to update duration value field
        function updateDurationValue() {
            const row = this.closest('.summaryRow');
            const fromHours = row.querySelector('select[name="duration_from_hours[]"]')?.value || '0h';
            const fromMinutes = row.querySelector('select[name="duration_from_minutes[]"]')?.value || '0m';
            const toHours = row.querySelector('select[name="duration_to_hours[]"]')?.value || '0h';
            const toMinutes = row.querySelector('select[name="duration_to_minutes[]"]')?.value || '0m';
            
            const durationValueField = row.querySelector('.duration-value');
            if (durationValueField) {
                durationValueField.value = `${fromHours} ${fromMinutes} to ${toHours} ${toMinutes}`;
            }
        }

        // Function to update all duration values before form submission
        function updateAllDurationValues() {
            document.querySelectorAll('.duration-select').forEach(select => {
                updateDurationValue.call(select);
            });
        }
        ...
        // Update all duration values before form submission
        summaryForm.addEventListener('submit', function(e) {
            updateAllDurationValues();
        });
        ...
        function enableEdit(button) {
            ...
            row.querySelectorAll('select[name="name"], textarea[name="value"], select[name="display"], select[name="duration_from_hours"], select[name="duration_from_minutes"], select[name="duration_to_hours"], select[name="duration_to_minutes"]').forEach(field => {
            ...
            // Add event listeners to duration selects for updating the hidden value
            const durationSelects = row.querySelectorAll('.duration-select');
            if (durationSelects.length > 0) {
                durationSelects.forEach(select => {
                    select.addEventListener('change', updateDurationValueInRow);
                });
                
                // Also update on form submit
                form.addEventListener('submit', updateDurationValueInRow);
            }
        }

        // Function to update duration value in a table row
        function updateDurationValueInRow() {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Add event listeners to existing duration selects
            document.querySelectorAll('.duration-select').forEach(select => {
                select.addEventListener('change', updateDurationValue);
            });
