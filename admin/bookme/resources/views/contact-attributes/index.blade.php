<x-app-layout>
    <div class="container w-[95%] mx-auto">
       ...
       <div class="flex item-center justify-between w-[90%] mx-auto pt-8">
            <h1 class="text-2xl font-bold ">Contact Attributes</h1>

            <div class="float-right ">

                <!-- Button to Open Modal for Adding Contact Attributes -->
        ...
        <!-- Flowbite Modal for Adding Contact Attributes -->
        <div id="static-modal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <!-- Modal body -->
                    <form action="{{ route('contact-attributes.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                            ...
                            <div id="summaryFields">
                                <div class="summaryRow grid grid-cols-2 gap-4 mb-4">
                                    <div class="form-group">
                                        <label for="attribute_name" class="block text-sm font-medium text-gray-700">Attribute Name<span class="text-red-500 font-bold text-2xl">*</span></label>
            ...
            <table id="example" class="table-auto w-full mt-6 border-collapse">
                ...
                <tbody>
                    ...
                    <tr class="bg-white hover:bg-gray-50">
                        <form action="{{ route('contact-attributes.update', $attribute->contact_id) }}" method="POST" class="update-form">
                            @csrf
                            @method('PUT')

                            <td>
                                <textarea name="id" class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $attribute->contact_id }}</textarea>
                            </td>
                            <td>
                                <textarea name="attribute_name" class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $attribute->attribute_name }}</textarea>
                            </td>
                            <td>
                                <textarea name="value" class="w-full border border-gray-300 rounded px-2 py-1 resize-none" disabled>{{ $attribute->value }}</textarea>
                            </td>
                            ...
                            <td class="flex space-x-2">
                                ...
                                <!-- Delete Button -->
                                <form action="{{ route('contact-attributes.destroy', $attribute->contact_id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
